...
function App() {
  ...
  async function startCase() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Could not open the case.");
  ...
  async function askQuestion(event) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "The detective could not answer.");
  ...
  async function submitAccusation(event) {
    ...
    try {
      ...
      const data = await response.json(); if (!response.ok) throw new Error(data.error || "Could not submit accusation.");
  ...
  return <main className="app-shell">
    ...
    <div className="workspace">
      ...
      <section className="detective-column"><div className="detective-area panel"><div className="lamp-glow" /><PlaceholderDetective /><div className="detective-copy"><span className="eyebrow">FIRST MATE SALTY SABLE</span><h1>Ask the right question.</h1><p>“The sea keeps its secrets, detective. We’ll see which ones this crew forgot to bury.”</p></div></div><div className="chat panel"><SectionTitle eyebrow="INTERROGATION LOG" title="Question the detective" /><div className="transcript">{messages.length === 0 && <div className="empty-log">Your questions will appear here. Choose your words carefully.</div>}{messages.map((message, index) => <div className={`message ${message.role}`} key={`${message.role}-${index}`}><span>{message.role === "player" ? "YOU" : "SALTY SABLE"}</span><p>{message.text}</p></div>)}</div><form className="question-form" onSubmit={askQuestion}><input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Ask about a suspect, clue, or moment..." disabled={busy || questionsRemaining === 0} /><button type="submit" disabled={busy || !draft.trim()}>SEND <span>↗</span></button></form>{error && <p className="error">{error}</p>}</div></section>
